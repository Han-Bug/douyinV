...
func (p *FavoriteActionResponse) fastWriteField1(buf []byte, binaryWriter bthrift.BinaryWriter) int {
	offset := 0
	offset += bthrift.Binary.WriteFieldBegin(buf[offset:], "base_rep", thrift.STRUCT, 1)
	offset += p.BaseRep.FastWriteNocopy(buf[offset:], binaryWriter)
	offset += bthrift.Binary.WriteFieldEnd(buf[offset:])
	return offset
}
...
func (p *FavoriteActionResponse) field1Length() int {
	l := 0
	l += bthrift.Binary.FieldBeginLength("base_rep", thrift.STRUCT, 1)
	l += p.BaseRep.BLength()
	l += bthrift.Binary.FieldEndLength()
	return l
}
